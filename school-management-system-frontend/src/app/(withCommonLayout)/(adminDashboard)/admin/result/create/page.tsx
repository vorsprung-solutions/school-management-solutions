...
import { useCreateResultMutation } from "@/redux/features/result/resultApi";
...
export default function CreateResultPage() {
  ...
  const calculateGPA = () => {
    if (watchedResults.length === 0) return 0;
  ...
  const onSubmit = async (data: CreateResultData) => {
    try {
      ...
      toast.error(error?.data?.message || "Failed to create result!");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="p-6">
      <div className="flex items-center gap-4 mb-6">
        ...
        <h1 className="text-2xl font-bold">Create New Result</h1>
      </div>
                      ...
                      {...register(`results.${index}.marks` as const, {
                        ...
                        onChange: (e) => {
                          ...
                          if (value === "" || isNaN(Number(value))) {
                            setValue(`results.${index}.marks` as const, 0);
                          } else {
                            setValue(
                              `results.${index}.marks` as const,
                              Number(value)
                            );
                          }
                        },
                      ...
                      <Input
                        {...register(`results.${index}.gpa` as const, {
                          ...
                          onChange: (e) => {
                            const value = e.target.value;
                            if (value === "" || isNaN(Number(value))) {
                              setValue(`results.${index}.gpa` as const, 0);
                            } else {
                              setValue(
                                `results.${index}.gpa` as const,
                                Number(value)
                              );
                            }
                          },
