...
import { useGetAllExamsQuery, useCreateExamMutation, useUpdateExamMutation, useDeleteExamByIdMutation, useRestoreExamByIdMutation } from "@/redux/features/exam/examApi";
...
export default function ExamPage() {
  ...
  const { data: examsData, isLoading, refetch } = useGetAllExamsQuery(undefined);
  
  const [createExam, { isLoading: isCreating }] = useCreateExamMutation();
  const [updateExam, { isLoading: isUpdating }] = useUpdateExamMutation();
  const [deleteExam, { isLoading: isDeleting }] = useDeleteExamByIdMutation();
  const [restoreExam, { isLoading: isRestoring }] = useRestoreExamByIdMutation();

  const { register, handleSubmit, reset, formState: { errors } } = useForm<CreateExamData>();
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      toast.error(error?.data?.message || "Failed to move exam to trash!");
    }
  };
  ...
  const handleRestore = async (id: string) => {
    try {
      ...
      toast.error(error?.data?.message || "Failed to restore exam!");
    }
  };
             ...
             <Table>
               ...
               <TableBody>
                 {exams.map((exam: IExam) => (
                   <TableRow key={exam._id} className={exam.isDeleted ? "opacity-60" : ""}>
         ...
         <AlertDialogContent>
           <AlertDialogHeader>
             ...
             <AlertDialogDescription>
               ...
               <span className="font-semibold">&quot;{examToDelete?.examName}&quot;</span> to trash. You can restore it later if needed.
             </AlertDialogDescription>
           </AlertDialogHeader>
           <AlertDialogFooter>
